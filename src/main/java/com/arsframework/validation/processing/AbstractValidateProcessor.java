...
public abstract class AbstractValidateProcessor extends AbstractProcessor {
     ...
     * @return true/false
     */
    protected boolean isIgnorable(Symbol.VarSymbol param, Class<? extends Annotation> annotation) {
        Ignore ignore;
    ...
    protected void buildValidateBlock(Symbol.VarSymbol param, Class<? extends Annotation> annotation) {
        ...
        if (!this.isIgnorable(param, annotation)
                && (condition = this.buildValidateCondition(param, annotation)) != null) {
            this.appendValidateBlock((Symbol.MethodSymbol) param.owner, Collections.singletonList(condition));
        }
    }
    ...
    private void appendValidateBlock(Symbol.MethodSymbol method, List<JCTree.JCStatement> conditions) {
        ...
        if (method.isConstructor() && this.isConstructorInvocation(body.stats.head)) {
            ListBuffer<JCTree.JCStatement> stats = ListBuffer.of(body.stats.head);
